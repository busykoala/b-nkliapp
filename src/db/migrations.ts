...
export function executeMigration(database: Database.Database, migration: Migration) {
  if (migration.unsafe) database.unsafeMode(true);
  try {
    database.exec(migration.sql);
  } finally {
    if (migration.unsafe) database.unsafeMode(false);
  }
}
...
export const migrations: Migration[] = [
  ...
  },
  {
    ...
    // This only broadens an existing CHECK expression. Rebuilding the table
    // would copy the multi-gigabyte national geometry dataset during startup.
    unsafe: true,
    sql: `
      ...
      SET sql=replace(
        sql,
        'CHECK(kind IN (''building'',''tree'',''water'',''forest'',''path'',''major_road''))',
