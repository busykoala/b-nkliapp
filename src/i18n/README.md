...
The app supports exactly four UI languages: Swiss Standard German (`de-CH`),
French (`fr-CH`), Italian (`it-CH`) and Rumantsch Grischun (`rm-CH`). German is
the fallback for an unsupported browser language. The menu saves an explicit
choice in the `benchly_language` cookie for one year.

An independent dialect toggle is stored in `benchly_dialect`. It never changes
navigation, account UI or pages without an opened bench. Inside a bench panel,
the complete information scope switches to the language and cautiously curated
regional variety of that bench's location; see `docs/dialect-mode.md`. URLs stay
the same, preserving links and map state.
