...
export async function getFeedPage(cursor: FeedCursor | null = null, limit = 48, scope: FeedScope = "all"): Promise<FeedPage> {
  ...
  const rows = sqlite.prepare(`
    SELECT * FROM (
      SELECT 'moment-' || m.id id, 'moment' kind, u.username, u.avatar_seed, b.id bench_id,
        coalesce(nullif(b.name,''), nullif(b.location_name,''), nullif(nullif(b.description,'Sitzbank'),'')) bench_name,
      ...
      SELECT 'care-' || c.id, 'care', u.username, u.avatar_seed, b.id,
        coalesce(nullif(b.name,''), nullif(b.location_name,''), nullif(nullif(b.description,'Sitzbank'),'')), c.created_at, c.kind, b.row_id, b.location_key
      ...
      SELECT 'added-' || b.row_id, 'added', u.username, u.avatar_seed, b.id,
        coalesce(nullif(b.name,''), nullif(b.location_name,''), nullif(nullif(b.description,'Sitzbank'),'')), b.imported_at, NULL, b.row_id, b.location_key
      ...
      SELECT 'rated-' || r.id, 'rated', u.username, u.avatar_seed, b.id,
        coalesce(nullif(b.name,''), nullif(b.location_name,''), nullif(nullif(b.description,'Sitzbank'),'')), r.updated_at, r.note, b.row_id, b.location_key
      ...
      SELECT 'confirmed-' || c.bench_row_id || '-' || c.user_id, 'confirmed', u.username, u.avatar_seed, b.id,
        coalesce(nullif(b.name,''), nullif(b.location_name,''), nullif(nullif(b.description,'Sitzbank'),'')), c.created_at, NULL, b.row_id, b.location_key
      ...
      SELECT 'missing-' || rc.request_id || '-' || rc.user_id, 'missing', u.username, u.avatar_seed, b.id,
        coalesce(nullif(b.name,''), nullif(b.location_name,''), nullif(nullif(b.description,'Sitzbank'),'')), rc.created_at, NULL, b.row_id, b.location_key
      ...
      SELECT 'edited-' || e.id, 'edited', u.username, u.avatar_seed, b.id,
        coalesce(nullif(b.name,''), nullif(b.location_name,''), nullif(nullif(b.description,'Sitzbank'),'')), e.created_at, e.field, b.row_id, b.location_key
  ...
  const entries = rows.slice(0, safeLimit).map((row) => {
    ...
    return {
      ...
      benchId: String(item.bench_id), benchName: item.bench_name == null ? null : String(item.bench_name), createdAt: String(item.created_at), detail: item.detail ? String(item.detail) : null,
    };
  });
...
function weeklyBench(): WeeklyBench | null {
  ...
  const row = sqlite.prepare(`
    SELECT id,coalesce(nullif(name,''),nullif(nullif(description,'Sitzbank'),''),nullif(location_name,'')) name,location_name place
    FROM benches WHERE active=1 AND verification_status='verified' ORDER BY row_id LIMIT 1 OFFSET ?
  `).get(offset) as { id: string; name: string | null; place: string | null } | undefined;
  return row ?? null;
}
