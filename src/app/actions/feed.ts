...
export async function getActivityFeed(limit = 60): Promise<FeedEntry[]> {
  ...
  return sqlite.prepare(`
    SELECT * FROM (
      SELECT 'added-' || b.row_id id, 'added' kind, u.username, b.id bench_id,
      ...
      WHERE b.active=1
      UNION ALL
      SELECT 'rated-' || r.id, 'rated', u.username, b.id,
      ...
      UNION ALL
      SELECT 'confirmed-' || c.bench_row_id || '-' || c.user_id, 'confirmed', u.username, b.id,
      ...
      WHERE b.active=1
      UNION ALL
      SELECT 'missing-' || rc.request_id || '-' || rc.user_id, 'missing', u.username, b.id,
      ...
      UNION ALL
      SELECT 'edited-' || e.id, 'edited', u.username, b.id,
  ...
  `).all(safeLimit).map((row) => {
    ...
    return {
      id: String(item.id), kind: String(item.kind) as FeedEntry["kind"], username: String(item.username),
