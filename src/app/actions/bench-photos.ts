...
export async function loadBenchPhoto(momentId: number): Promise<BenchPhotoResult> {
  ...
  if (!parsed.success) return { ok: false, message: "Bild nicht gefunden." };
  ...
  if (!moment?.photo_url?.startsWith("garage:")) return { ok: false, message: "Bild nicht gefunden." };
  try {
    const photo = await readBenchPhoto(moment.photo_url);
    if (!photo || !allowedTypes.has(photo.contentType)) return { ok: false, message: "Bild nicht gefunden." };
    return { ok: true, dataUrl: `data:${photo.contentType};base64,${Buffer.from(photo.bytes).toString("base64")}` };
  } catch {
    return { ok: false, message: "Bild gerade nicht verfügbar." };
  }
}
...
export async function uploadBenchPhoto(benchId: string, formData: FormData): Promise<ActionResult> {
  try {
    ...
    if (!(photo instanceof File) || !allowedTypes.has(photo.type) || photo.size < 8_000 || photo.size > 1_800_000) {
      return { ok: false, message: "Bitte wähle ein klares Foto bis 1,8 MB." };
    }
    ...
    try {
      sqlite.prepare(`INSERT INTO bench_moments (bench_row_id,user_id,kind,body,photo_url,created_at,updated_at,visible)
        VALUES (?,?,'photo',?,?,?,?,1)`).run(bench.row_id, user.id, parsed.caption || "Ein Blick von diesem Bänkli.", url, now, now);
    } catch (error) {
      await deleteBenchPhoto(url).catch(() => undefined);
      throw error;
    }
    revalidatePath(`/bank/${id}`); revalidatePath("/feed");
    return { ok: true, message: "Foto angekommen – das Bänkli hat jetzt sein Porträt." };
  } catch (error) {
    return { ok: false, message: error instanceof Error ? error.message : "Das Foto konnte nicht gespeichert werden." };
  }
}
