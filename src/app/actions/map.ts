...
export async function getBenchDetail(benchId: string): Promise<BenchDetail | null> {
  ...
  const exactOsmEvidence = Boolean(sqlite.prepare(`
    ...
    WHERE kind IN ('import-osm','refresh') AND status='completed' AND pipeline_version IN ('4.2.0','4.3.0','4.4.0')
    LIMIT 1
  `).get());
  ...
  const needsContextRefresh = !hasTerrainModel || !["GeoAdmin-Horizont v6", "4.4.0"].includes(pipelineVersion)
  ...
  if (needsContextRefresh) {
    ...
    if (terrainEvidence && contextModel) {
      ...
      pipelineVersion = "GeoAdmin-Horizont v6";
      ...
      cacheEnrichment(() => sqlite.prepare(`
        INSERT INTO bench_enrichments (
          ...
          @contextSourceVersion,'GeoAdmin-Horizont v6',@computedAt,'mittel',
  ...
  return {
    ...
    sunConfidence: (hasTerrainModel ? contextModel ? exactOsmEvidence ? "mittel" : "niedrig"
      : pipelineVersion === "GeoAdmin-Horizont v6" && !exactOsmEvidence ? "niedrig" : row.sun_confidence ?? "mittel" : "niedrig") as BenchDetail["sunConfidence"],
