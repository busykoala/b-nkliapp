  ...
  return <aside className={`journey-panel storybook-panel ${expanded ? "is-expanded" : ""}`} aria-label="Dein Weg zum Bänkli">
    ...
    <div className="journey-scroll">
      ...
      {result && <section ref={resultSection} className="journey-results" aria-label="Deine Verbindungen">
        ...
        {option && <div className="journey-recommendation"><span className="story-eyebrow">Deine Verbindung</span><h3>{journeyClock(option.arrival)} am Ziel</h3><p className="journey-walking-summary"><Footprints size={18} /> {journeyMinutes(option.walkingSeconds)} zu Fuss insgesamt</p>{finalWalk && <p>Letzter Fussweg: {finalWalk.geometryQuality === "missing" ? "noch unbekannt" : journeyMinutes(finalWalk.durationSeconds)}{finalWalk.distanceMeters !== undefined ? ` · ${Math.round(finalWalk.distanceMeters)} m` : ""}</p>}<p>{journeyMinutes(option.durationSeconds)} Reisezeit · {option.changes} Umstiege</p><details><summary>Umsteigen</summary><p>{tightestTransfer(option)}</p></details>{(!option.complete || !option.feasible) && <p className="journey-warning">Noch nicht vollständig geprüft</p>}</div>}
        {links && <div className="journey-external-links">{links.sbb && <a href={links.sbb} target="_blank" rel="noreferrer">In SBB öffnen ↗</a>}{links.maps && <a href={links.maps} target="_blank" rel="noreferrer">In Karten öffnen ↗</a>}{links.sbb && <small>SBB übernimmt den ÖV-Teil. Karten zeigt den Weg bis zum Ziel.</small>}</div>}
        {result.options.length > 1 && <details><summary>Weitere Verbindungen</summary><div className="journey-alternatives">{result.options.filter((o) => o.id !== selected).map((o) => <button key={o.id} onClick={() => selectOption(o)}><strong>{journeyClock(o.arrival)} am Ziel</strong><span>{journeyMinutes(o.walkingSeconds)} zu Fuss · letzter Fussweg {finalWalkingLeg(o)?.geometryQuality === "missing" ? "unbekannt" : journeyMinutes(finalWalkingLeg(o)?.durationSeconds ?? 0)}</span><small>{journeyMinutes(o.durationSeconds)} Reisezeit · {o.changes} Umstiege</small>{(!o.complete || !o.feasible) && <small>Noch nicht vollständig geprüft</small>}</button>)}</div></details>}
