...
import { Accessibility, Armchair, Building2, CircleCheck, Clock3, Compass, ExternalLink, EyeOff, Flag, Hammer, Image as ImageIcon, Info, Leaf, MapPin, Moon, Mountain, MountainSnow, MoveHorizontal, Route, Star, Sun, Sunrise, Sunset, Telescope, TreePine, Trees, Umbrella, UsersRound, Waves } from "lucide-react";
    ...
    <div className="pb-8">
      <div className="mb-4">
        <div className="flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-wide">
          ...
          {bench.analysisCoverage === "near-field"
            ? <span className="badge badge-primary gap-1"><Telescope size={14} /> Aussicht wird ergänzt</span>
            : <span className="badge badge-primary gap-1"><Mountain size={14} /> Aussicht {bench.viewScore ?? "–"}/5</span>}
        ...
        <p className="mt-1 flex items-center gap-1 text-sm opacity-65"><MapPin size={15} /> {bench.latitude.toFixed(5)}, {bench.longitude.toFixed(5)} · {bench.elevationMeters !== null ? `${Math.round(bench.elevationMeters)} m ü. M.` : "Höhe derzeit nicht verfügbar"}</p>
      </div>
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-box border border-warning/40 bg-warning/8 p-4">
      ...
      <div className="my-3 rounded-xl bg-base-100/80 p-3">
        <div className="flex items-center justify-between gap-3"><span className="flex items-center gap-2 text-sm font-semibold"><Clock3 size={17} className="text-primary" /> {bench.analysisCoverage === "near-field" ? "Geschätzte Sonne heute" : "Direkte Sonne heute"}</span><strong>{bench.directSunrise === "Noch nicht berechnet" ? "–" : sunDuration(bench.sunMinutesToday)}</strong></div>
        <div className="mt-2 flex flex-wrap gap-2">{bench.sunWindows.length ? bench.sunWindows.map((window) => <span className="badge badge-warning badge-outline" key={`${window.start}-${window.end}`}>{window.start}–{window.end}</span>) : <span className="text-sm opacity-65">{bench.analysisCoverage === "near-field" ? "Im Nahbereichsmodell heute kein freies Sonnenfenster" : "Heute kein berechnetes Sonnenfenster"}</span>}</div>
      </div>
      ...
      <ModelStatus coverage={bench.analysisCoverage} value={bench.sunConfidence} kind="sun" />
      <p className="mt-2 text-xs leading-relaxed opacity-65">{bench.analysisCoverage === "near-field" ? "Nahbereichsschätzung im 5-Minuten-Raster: Erfasste Gebäude und Bäume bis 350 m werden mit Höhe, Winkelbreite und Sonnenrichtung berücksichtigt. Der Berg- und Geländehorizont fehlt noch; besonders erste und letzte Sonne können deshalb abweichen." : "Direkte Sonne wird im 5-Minuten-Raster gegen Gelände, Gebäudeoberflächen, Baumkronen und Überdachung geprüft."} Wolken, Laubwechsel und temporäre Objekte sind nicht berücksichtigt.</p>
    </section>
    <section className="rounded-box bg-primary/8 p-4"><h3 className="flex items-center gap-2 font-bold"><Telescope size={19} /> {bench.analysisCoverage === "near-field" ? "Aussicht im Nahbereich" : `Aussicht ${bench.viewScore ?? "–"}/5`}</h3><div className="mt-3 flex flex-wrap gap-2">{bench.viewLabels.map((label) => <span className="badge badge-primary badge-outline gap-1" key={label}>{viewIcon(label)}{label}</span>)}</div><ul className="mt-3 space-y-2 text-sm">{bench.viewExplanation.filter((item) => !bench.viewLabels.includes(item)).map((item) => <li className="flex gap-2" key={item}><CircleCheck className="mt-0.5 shrink-0 text-primary" size={16} aria-hidden />{item}</li>)}</ul><div className="mt-3"><ModelStatus coverage={bench.analysisCoverage} value={bench.viewConfidence} kind="view" /></div>{bench.directionDegrees !== null && <p className="mt-2 flex items-center gap-2 text-sm"><Compass size={16} /> Blickrichtung etwa {Math.round(bench.directionDegrees)}°</p>}</section>
    <section><h3 className="mb-2 flex items-center gap-2 font-bold"><Leaf size={19} /> Umgebung</h3><div className="grid grid-cols-2 gap-2"><InfoCell icon={<Trees />} label="Vegetation" value={bench.inForest ? `Wald · ${Math.round(bench.canopyPercent ?? 0)}% Krone` : `Offen · ${Math.round(bench.canopyPercent ?? 0)}% Krone`} /><InfoCell icon={<Waves />} label="Wasser" value={distance(bench.distanceWaterMeters)} /><InfoCell icon={<Route />} label="Nächster Weg" value={distance(bench.distancePathMeters)} /><InfoCell icon={<Building2 />} label="Nächstes Gebäude" value={distance(bench.distanceBuildingMeters)} /><InfoCell icon={<Building2 />} label="Gebäude in 100 m" value={bench.buildingCount100m === null ? "Unbekannt" : String(bench.buildingCount100m)} /><InfoCell icon={<Mountain />} label="Höhe" value={bench.elevationMeters !== null ? `${Math.round(bench.elevationMeters)} m ü. M.` : "Nicht verfügbar"} /></div></section>
...
function ModelStatus({ value, coverage, kind }: { value: BenchDetail["viewConfidence"]; coverage: BenchDetail["analysisCoverage"]; kind: "sun" | "view" }) { return <span className="badge badge-outline text-xs">{coverage === "near-field" ? (kind === "sun" ? "Nahbereichsschätzung" : "Nahbereich analysiert") : `Sicherheit: ${value}`}</span>; }
