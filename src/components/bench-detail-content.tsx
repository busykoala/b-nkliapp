    ...
    <div className="pb-8">
      <div className="mb-4">
        <div className="flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-wide">
          <span className={`badge gap-1 ${bench.sunnyNow ? "badge-warning" : "badge-ghost"}`}><Sun size={14} />{bench.sunnyNow === null ? "Sonne noch nicht analysiert" : bench.sunnyNow ? "Sonne jetzt" : `Schatten: ${shadeLabel(bench.shadeCause)}`}</span>
...
function Details({ bench }: { bench: BenchDetail }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-box border border-warning/40 bg-warning/8 p-4">
      ...
      <p className="mt-2 text-xs leading-relaxed opacity-65">Direkte Sonne wird im 5-Minuten-Raster gegen Gelände, Gebäudeoberflächen, Baumkronen und Überdachung geprüft. Wolken, Laubwechsel und temporäre Objekte sind nicht berücksichtigt.</p>
    </section>
...
function Confidence({ value }: { value: BenchDetail["viewConfidence"] }) { return <span className="badge badge-outline text-xs">Sicherheit: {value}</span>; }
