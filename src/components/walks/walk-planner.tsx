  ...
  return <aside className={`journey-panel storybook-panel ${expanded ? "is-expanded" : ""}`} aria-label={t("walks.planner.title")}>
    ...
    <div className="journey-scroll"><header><span className="story-eyebrow">{t("walks.planner.eyebrow")}</span><h2 ref={title} tabIndex={-1}>{t("walks.planner.title")}</h2><p>{t("walks.planner.intro")}</p></header>
      ...
      {p.chosen && p.result && copy && <section className="journey-results" aria-label={t("walks.results.title")}>
        ...
        <details><summary>{t("walks.results.why")}</summary><p>{t("walks.results.explanation")}</p>{p.chosen.evidence.warnings.map((warning, index) => <p key={`${warning.key}-${index}`}>{translateMessage(t, warning)}</p>)}<p>{p.chosen.evidence.updatedAt ? t("walks.results.updated", {date: formatDate(p.chosen.evidence.updatedAt, t)}) : t("walks.results.noData")}  {t("walks.results.extrasNote")}</p></details>
