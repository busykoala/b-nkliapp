...
export function StartPicker({ origin, onChange, getMap }: { origin: JourneyOrigin | null; onChange: (origin: JourneyOrigin | null) => void; getMap: () => MapLibreMap | null }) {
  ...
  const choose = (p: JourneyOrigin) => { sequence.current++; setResults([]); setEditing(false); setMessage(null); onChangeRef.current(p); };
  useEffect(() => {
    const ref = sequence;
    const map = getMap();
  ...
  // A server action can refresh the translation provider when setting its
  // anonymous cookie. That must not repeat a search or reset keyboard selection.
  useEffect(() => {
    ...
    const timer = setTimeout(() => startTransition(async () => {
      ...
      catch { if (sequence.current === token) setMessage({ key: "routing.origin.searchFailed" }); }
    }), 350);
