...
export function AccountDialog({ dialogRef }: { dialogRef: React.RefObject<HTMLDialogElement | null> }) {
  ...
  return <dialog ref={dialogRef} className="modal modal-bottom sm:modal-middle" aria-labelledby={titleId} aria-describedby={descriptionId}>
    <div className="modal-box storybook-sheet max-h-[calc(100dvh-env(safe-area-inset-top)-1rem)] overflow-y-auto rounded-t-[2rem] pb-[max(1rem,env(safe-area-inset-bottom))] sm:rounded-[2rem]">
      ...
      <AccountForm key={mode} mode={mode} onSuccess={() => { dialogRef.current?.close(); router.refresh(); }} />
...
function AccountForm({ mode, onSuccess }: { mode: "login" | "register"; onSuccess: () => void }) {
  const [state, formAction, pending] = useActionState(mode === "login" ? login : register, null);
