...
export function AddBenchDialog({ open, coordinates, onClose }: { open: boolean; coordinates: { latitude: number; longitude: number }; onClose: () => void }) {
  ...
  return <dialog ref={ref} onClose={onClose} className="modal modal-bottom sm:modal-middle">
    <div className="modal-box storybook-sheet rounded-t-[2rem] sm:rounded-[2rem]">
      <button aria-label="Schliessen" className="btn btn-circle btn-ghost absolute right-3 top-3" onClick={onClose}><X size={19} /></button>
      ...
      <p className="mt-1 flex items-center gap-1 text-sm opacity-60"><MapPin size={15} /> Die Kartenmitte wird als Standort verwendet.</p>
