...
export function readBenchDetail(benchId: string, currentUser: CurrentUser | null): BenchDetail | null {
  ...
  return {
    ...
    ratingBreakdown: ratingCount ? {
      overall: Number(Number(row.rating_average).toFixed(1)),
      view: row.rating_view === null ? null : Number(Number(row.rating_view).toFixed(1)),
      comfort: row.rating_comfort === null ? null : Number(Number(row.rating_comfort).toFixed(1)),
      quiet: row.rating_quiet === null ? null : Number(Number(row.rating_quiet).toFixed(1)),
      counts: { overall: ratingCount, view: Number(row.rating_view_count), comfort: Number(row.rating_comfort_count), quiet: Number(row.rating_quiet_count) },
    } : null,
