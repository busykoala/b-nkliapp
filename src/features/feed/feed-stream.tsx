  ...
  return <div aria-busy={pending}>
    {groups.map((group) => {
      ...
      return <article className="feed-bench-group" key={group.key}>
        <header><Rss size={18} /><div><h2><Link href={`/bank/${group.benchId}`}>{group.benchName ?? t("common.values.bench")}</Link></h2><time dateTime={group.date}>{formatDate(group.entries[0].createdAt, t, "long")}</time></div><Link className="feed-arrow" href={`/bank/${group.benchId}`} aria-label={t("feed.stream.openBench", { name: group.benchName ?? t("common.values.bench") })}>→</Link></header>
