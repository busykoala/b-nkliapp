...
describe("Bänkli-centred outing copy", () => {
  it("keeps actual names and counts optional discoveries separately", () => {
    expect(walkCopy([bench], "loop", 7, testTranslator())).toEqual({ title: "Eine Runde zum Bänkli «Seeblick»", pause: "Mit dem Bänkli «Seeblick» zum Innehalten", discover: "Und unterwegs noch 7 weitere Bänkli zum Entdecken." });
  });
  it("only describes water when supported, and hides zero discoveries", () => {
    expect(walkCopy([{ ...bench, name: "Sitzbank" }], "loop", 0, testTranslator())).toMatchObject({ title: "Eine Runde zum Uferbänkli", discover: null });
    expect(walkCopy([{ ...bench, name: null, waterfront: false }], "one-way", 1, testTranslator())).toMatchObject({ title: "Ein Spaziergang zum Bänkli", pause: "Mit einem Bänkli zum Innehalten", discover: "Und unterwegs noch ein weiteres Bänkli zum Entdecken." });
  });
  it("deduplicates repeated legs and removes planned stops", () => {
    ...
    expect(walkCopy([bench, extra, { ...bench, id: "c" }], "loop", 2, testTranslator()).pause).toBe("Mit 3 Bänkli zum Innehalten");
