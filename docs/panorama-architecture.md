...
Stand: 2026-09-13. Dieser Vertrag beschreibt den v19-Cutover und trennt ausgelieferte Software von der noch laufenden nationalen Datenaufbereitung.

## Produktvertrag

Im Bänkli-Overlay gibt es keine generische Ersatzlandschaft mehr. Angezeigt werden ausschliesslich ein geografisches v19-WebP, ein während einer Erneuerung weiter nutzbares v19-WebP oder ein neutraler warmer Papierwash. Alte SVG-/v18-Artefakte und die frühere `BenchLandscape`-Komposition werden nicht mehr gelesen.

`PanoramaDescriptor` unterscheidet `ready`, `generating`, `stale`, `unavailable` und `error`. `stale` behält das letzte geografische v19-Bild sichtbar. Artefakte werden über eine gleichoriginige, schlüsselvalidierte Media-Route mit ETag und unveränderlichem Cache-Header gestreamt; Server Actions liefern nur Status und Metadaten.
...
## Aquarell-Renderer v19

Der Pillow-Renderer malt deterministisch auf einer weicheren internen Arbeitsfläche und skaliert mit Lanczos auf 4096 × 1024. Diese Auflösung liefert im etwa 90° breiten UI-Crop mehr als genug Detail und hält den lokalen Render bei ungefähr einer Sekunde. Der Cache-Schlüssel bindet Geometrie, Saison, Stil und Ausgabeformat ein.

Ferne Distanzschichten werden heller, kühler, entsättigter und weicher. Nahe Flächen bleiben wärmer und kontrastreicher. Mehrere leicht versetzte transparente Washes, globales niedrigfrequentes Pigment, Kantenakkumulation und einmaliges Papierkorn erzeugen die Malwirkung ohne wiederholte Texturkacheln. Felder sind ruhig; Wald ist flächig; Häuser sind kohärente Körper. Ein zusammenhängender erd-/sandfarbener Vordergrund garantiert sicheren Bodenkontakt und verhindert eine Bank im Wasser.
...
Migration `0033_panorama_artifacts_v2` ergänzt WebP-/Saison-/Manifest-/Vollständigkeitsfelder, Queue-Priorität, Lease, Retry-Zeit und eine Lichtkartentabelle. Geometrie liegt atomar als kompaktes NPZ, Basisbilder unter `renders-v19/`, Lichtkarten unter `lightmaps-v1/`; SQLite hält nur Zustand, Schlüssel, Provenienz und Pfade.
...
Geprüft werden 390 px, 430 px und 1440 px, endloses Ziehen, vertikaler Overscan, Bänklirichtung, 360°-Naht, Gebäude-/Wasser-/Waldmassen, innere Grate, atmosphärische Tiefe, Bodenkontakt, Wetter, Mondphase und Schattenrichtung. Synthetische Tests decken Tiefenverdeckung, Geländegrate, deterministische WebPs, Lichtkarten, niedrige/hohe Sonne und Bewölkungsreduktion ab.
